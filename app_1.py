...
def create_interactive_chart(df: pd.DataFrame, id_col: str, title: str, chart_type: str = "auto"):
    ...
    if chart_type == "sunburst":
        ...
        create_radar_chart(df, id_col, numeric_cols, title)
...
def create_radar_chart(df, id_col, numeric_cols, title):
    """Enhanced radar chart for multi-dimensional comparison"""
    fig = go.Figure()
    
    for i, row in df.head(5).iterrows():  # Limit to 5 items for readability
        values = [row[col] for col in numeric_cols[:8]]  # Max 8 dimensions
        fig.add_trace(go.Scatterpolar(
            r=values,
            theta=numeric_cols[:8],
            fill='toself',
            name=str(row[id_col]),
            opacity=0.6
        ))
    ...
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, max([df[col].max() for col in numeric_cols])])),
        showlegend=True,
        title=title,
        height=600
    )
    st.plotly_chart(fig, use_container_width=True)
    st.info(f"🕸️ Radar chart comparing top 5 {id_col} across {len(numeric_cols)} dimensions")
...
def agency_table_with_filters(df: pd.DataFrame):
    ...
    with col1:
        st.info(f"📊 Showing {len(page_df) if total_rows > 0 else 0} of {total_rows:,} rows")
    
    with col2:
        if st.button("⬅️ Previous", disabled=(st.session_state.agency_page <= 1)):
            st.session_state.agency_page -= 1
            st.rerun()
    ...
    with col4:
        if st.button("Next ➡️", disabled=(st.session_state.agency_page >= total_pages)):
            st.session_state.agency_page += 1
            st.rerun()
    ...
    # Status display
    st.markdown(f"**Page {st.session_state.agency_page} of {total_pages}** • **{total_rows:,} total rows**")
    
    # Download button
    if total_rows > 0:
        csv_data = display_filtered.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Download Filtered Data", csv_data, 
                          "agency_filtered_data.csv", "text/csv")
...
def plot_victim_analysis():
    if "Victims Age" in loaded_data:
        ...
        if id_col:
            create_interactive_chart(df, id_col, "🧓 Victims by Age Category", "radar")
    if "Victims Sex" in loaded_data:
        df = loaded_data["Victims Sex"]
        id_col = next((c for c in df.columns if "offense" in c.lower() and "category" in c.lower()),
                      df.columns[0] if len(df.columns) > 0 else None)
        if id_col:
            create_interactive_chart(df, id_col, "👫 Victims by Sex", "sunburst")
    if "Victims Race" in loaded_data:
        df = loaded_data["Victims Race"]
        id_col = next((c for c in df.columns if "offense" in c.lower() and "category" in c.lower()),
                      df.columns[0] if len(df.columns) > 0 else None)
        if id_col:
            create_interactive_chart(df, id_col, "🌍 Victims by Race", "treemap")
...
def plot_offender_analysis():
    if "Offenders Age" in loaded_data:
        ...
        if id_col:
            create_interactive_chart(df, id_col, "👤 Offenders by Age Category", "heatmap")
    if "Offenders Sex" in loaded_data:
        df = loaded_data["Offenders Sex"]
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "⚖️ Offenders by Sex", "grouped_bar")
    if "Offenders Race" in loaded_data:
        df = loaded_data["Offenders Race"]
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "🌐 Offenders by Race", "sunburst")
...
def plot_arrestee_analysis():
    if "Arrestees Age" in loaded_data:
        ...
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "🚓 Arrestees by Age Category", "radar")
    if "Arrestees Sex" in loaded_data:
        df = loaded_data["Arrestees Sex"]
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "👮 Arrestees by Sex", "treemap")
    if "Arrestees Race" in loaded_data:
        df = loaded_data["Arrestees Race"]
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "🔍 Arrestees by Race", "heatmap")
...
def plot_other_analysis():
    ...
    if "Property Crimes by Location" in loaded_data:
        ...
        id_col = df.columns[0] if len(df.columns)>0 else None
        if id_col:
            create_interactive_chart(df, id_col, "🏠 Property Crimes by Location", "heatmap")
...
if selected_group == "Geospatial / State-Level":
    ...
    if df is not None and not df.empty:
        ...
        if id_col:
            create_interactive_chart(df, id_col, "📊 Incidents & Offenses Analysis", "auto")
    else:
    ...
    if not time_related:
        ...
        for key, df in time_related:
            ...
            if id_col is None:
                ...
                id_col = df.columns[0] if len(df.columns) > 0 else None
            if id_col:
                create_interactive_chart(df, id_col, f"🕐 {pretty} — Time Analysis", "polar")
    ...
    for key, df in loaded_data.items():
        ...
        if "drug" in key.lower() or "alcohol" in key.lower():
            ...
            if id_col:
                create_interactive_chart(df, id_col, f"💉 {pretty_title_from_key(key)}", "grouped_bar")
    ...
    for fname in csv_files:
        if "drug" in fname.lower() or "alcohol" in fname.lower():
            ...
            if id_col:
                create_interactive_chart(df_try, id_col, f"💊 {pretty_title_from_key(fname)}", "treemap")
    if not found:
